...
lib.create = ((dir, file, data, callback ) => {
    ...
    fs.open(`${basePath}${dir}/${file}.json`, 'wx', (err, fileDescriptor) => {
        if (!err) {
            // Write the data to the opened file
            fs.writeFile(`${basePath}${dir}/${file}.json`, data, (err) => {                
                if (!err) {
                    callback(200, util.jsonParser(data));
                } else {
                    callback(500, 
...
lib.update = (( dir, file, userData, callback ) => {
    // Open the file
    fs.open(`${basePath}${dir}/${file}.json`, 'r+', (err, fileDescriptor) => {
        if (!err) {  
            // Pull existing data
            fs.truncate(`${basePath}${dir}/${file}.json`,(err => {
                if (err){
                    callback(500, util.errorUtility(200, 'Internal Server Error', 'fileProcessing'));
                } else {
                    // Write the data to the opened file
                    fs.writeFile(`${basePath}${dir}/${file}.json`, userData, (err, fileDescriptor) => {                
                        if (!err) {
                            callback(200,util.jsonParser(userData))
                        } else {
                            callback(500, 
                                util.errorUtility(500, 'Server Error: Could not write to file', 'fileProcessing')
                            );
